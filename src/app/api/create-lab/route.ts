	...
	try {
		...
		if (getEmailQuery?.rows[0]?.email) {
			const result = await pool.query(
				"INSERT INTO labs(estname,phoneNumber,location,userId,license_document_link,services) VALUES($1,$2,$3,$4,$5,$6) RETURNING id ",
				[
					name,
					phoneNumber,
					location,
			...
			const updateRoleQuery = await pool.query(`UPDATE users set userRole='lab' WHERE id=$1`, [getEmailQuery?.rows[0]?.id]);
			// eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
			if (result.rows[0].id && updateRoleQuery.rowCount == 1) {
				return Response.json({ status: "lab added" });
			} else {
				return Response.json({
					status: "An internal error adding the lab",
				});
			}
		} else {
			return Response.json({
